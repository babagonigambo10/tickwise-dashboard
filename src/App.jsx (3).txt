...
function Dashboard({ session }) {
  ...
  useEffect(() => {
    (async () => {
      ...
      const { count } = await supabase
        .from('api_credentials').select('id', { count: 'exact', head: true })
        .eq('user_id', userId).eq('exchange', cfg?.exchange || DEFAULT_CONFIG.exchange)
  ...
  useEffect(() => {
    ...
    (async () => {
      const { count } = await supabase
        .from('api_credentials').select('id', { count: 'exact', head: true })
        .eq('user_id', userId).eq('exchange', config.exchange)
      setHasCreds((count ?? 0) > 0)
    })()
...
function ControlPanel({ userId, config, setConfig, hasCreds, sessionRow }) {
  ...
  async function saveConfig() {
    ...
    setSaving(true)
    // Drop any id carried over from a previously-loaded config (e.g. from a
    // different symbol). If we don't, Postgres tries to insert a new row
    // using an id that already belongs to that other row, colliding with
    // the primary key. Letting Postgres/ON CONFLICT decide the id is safer.
    const { id, ...configWithoutId } = config
    const { data, error } = await supabase
      .from('bot_configs')
      .upsert(
        { ...configWithoutId, user_id: userId, min_competitor_value: minV, max_competitor_value: maxV },
  ...
  async function toggleBot() {
    ...
    try {
      ...
      // First-time users have a config only in memory (DEFAULT_CONFIG),
      // which has no id yet -- Postgres only assigns one on insert. Save
      // it first so Start doesn't fail with "Missing config_id" just
      // because Save was never clicked separately.
      let activeConfigId = config.id
      if (!isRunning && !activeConfigId) {
