...
function encrypt(text) {
  const key = Buffer.from(process.env.ENCRYPTION_KEY.trim(), 'base64')
  ...
  // IMPORTANT: supabase-js sends the request body as JSON. JSON.stringify()
  // on a raw Buffer produces {"type":"Buffer","data":[...]}, which Postgres
  // would then store literally as the bytea's bytes — not the ciphertext.
  // Converting to a Postgres bytea hex-input string ('\x' + hex) here avoids that.
