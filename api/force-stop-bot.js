...
export default async function handler(req, res) {
  ...
  try {
    const orchestratorRes = await fetch(`${ORCHESTRATOR_URL}/internal/force-stop`, {
      method: 'POST',
      headers: {
    ...
    if (!orchestratorRes.ok) {
      return res.status(orchestratorRes.status).json({ detail: body.detail || 'Force stop failed' })
    }
